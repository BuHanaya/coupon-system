...
function saveData() {
  localStorage.setItem(STORAGE_KEY, JSON.stringify({ coupons, seq }));
}
...
function genCode(prefix, s) {
  const p = prefix ? prefix.toUpperCase().replace(/[^A-Z0-9]/g, '') + '-' : '';
  return p + String(s).padStart(5, '0');
}
...
function generateCoupons() {
  ...
  const prefix = document.getElementById('g-prefix').value.trim();
  ...
  // reset form
  ['g-name','g-value','g-min','g-exp','g-prefix'].forEach(id => document.getElementById(id).value = '');
...
function renderTable() {
  ...
  let list = coupons.filter(c => {
    ...
    if (search &&
      ...
      !String(c.seq).includes(search)) return false;
    return true;
  });
...
function verifyCoupon() {
  ...
  if (!coupon) {
    res.className = 'verify-result v-invalid';
    res.style.display = 'block';
    res.innerHTML = `
      <div class="v-title">❌ الكوبون غير موجود</div>
  ...
  if (st === 'used') {
    ...
    res.style.display = 'block';
    res.innerHTML = `
      <div class="v-title">⚠️ الكوبون مستخدم مسبقًا</div>
      <div class="v-details">
        الكود: <span class="code-badge">${coupon.code}</span><br>
        تاريخ الاستخدام: ${coupon.usedAt || '—'}
      </div>`;
    return;
  }
  ...
  if (st === 'expired') {
    ...
    res.style.display = 'block';
    res.innerHTML = `
      <div class="v-title">❌ الكوبون منتهي الصلاحية</div>
      <div class="v-details">انتهت صلاحية هذا الكوبون في ${coupon.exp}</div>`;
    return;
  }

  res.className = 'verify-result v-valid';
  res.style.display = 'block';
  res.innerHTML = `
  ...
  document.getElementById('s-used-list').innerHTML = usedList.length
    ? usedList.map(c => `
        <div class="simple-row">
          <span class="code-badge" style="font-size:12px">${c.code}</span>
          <span style="color:var(--text3);font-size:12px">${c.usedAt || '—'}</span>
        </div>`).join('')
    : '<div style="color:var(--text3);font-size:13px;padding:1rem 0">لا يوجد كوبونات مستخدمة بعد</div>';

  const discounts = {};
  coupons.forEach(c => {
    const k = c.value + (c.type === 'percent' ? '%' : ' ريال');
    discounts[k] = (discounts[k] || 0) + 1;
  });

  document.getElementById('s-discount-list').innerHTML = Object.keys(discounts).length
    ? Object.entries(discounts).sort((a, b) => b[1] - a[1]).map(([k, v]) => `
        <div class="simple-row">
          <span style="font-family:var(--mono);font-size:13px">${k}</span>
          <span style="color:var(--text3);font-size:12px">${v} كوبون</span>
        </div>`).join('')
